...
def sendData(slaveAddress, data):
        bus.write_i2c_block_data(slaveAddress, data[0],data[1:])
...
class Stepper:
   def __init__(self, img_size):
      ...
      self.send_list = [0 for i in range(16)]
   ...
   def read(self):
      ...
      if bytes[0]==6:
         ...
         if check_sum==bytes[15]:
            ...
            self.mal_check+=1
            print("check malo",self.send_list)
            return False
      else:
         self.no_cmd+=1
         return False
...
def main():
   ...
   print("no se reconoce comando: ",spr.no_cmd)
